...
### PR #82 — 오늘 탭 데일리 입력 UX + 빠른 캡처 + 일기 단순화

- **한 줄 임팩트**: 하루 일을 오늘 탭에서 더 빨리 잡고 흘려보내지 않습니다
- 매일 쓰며 쌓인 마찰 일괄 정리. 빠른 추가에 날짜·시간을 그 자리에서 입력(박스 레이아웃, 메모 편집모드 패턴 재사용), 할 일 카운트는 잘린 10개가 아니라 실제 남은 개수, 다른 날 보기의 "오늘로"를 본문 버튼에서 헤더 날짜로 이동(클릭존 확대).
- 빠른 캡처 포스트잇. 날짜에 안 묶인 평문 메모를 오늘 사이드바 하단에 상시 고정(vault `scratchpad.md`), 위쪽 경계선 드래그로 높이조절.
- 일기를 마크다운 에디터에서 평문으로 단순화(실사용이 평문). 편집/보기 토글 제거.
- 저장 표시를 메모장 spinner 로 통일(`SaveIndicator` 공용 추출). 일기 저장 표시는 타이틀 옆으로.
- 좁은 창에서 타이틀바가 사라져 창 드래그가 막히던 것 수정. 바·버튼 유지, 날짜·시간만 숨김.

## 2026-06-18

### PR #79 — 캘린더 탭 폐기 → "오늘" 탭이 날짜 훑기·일기 진입 흡수 (SIMPLIFY 캘린더 강등)

- **한 줄 임팩트**: 오늘 한 화면에서 다른 날 일정·일기까지 훑습니다
- 캘린더 탭과 "오늘" 탭의 경계가 흐려 중복이었음 (둘 다 날짜·일정·일기). 캘린더 탭을 없애고 살릴 기능(날짜 훑기, 일기 진입)을 "오늘" 탭 사이드바로 흡수.
- 탭 4개로 (오늘/메모장/할일/포트폴리오, Cmd+1~4). `CalendarPage`·`MonthGrid`·`lib/calendar/spans` 삭제 (−1,600줄), 액티비티바 캘린더 아이콘 제거, `#calendar` → `#today` fallback.
- 일기 오버레이를 App 레벨로 — 메인 "오늘 일기" 블록 + 사이드바 날짜 클릭 + QuickSwitcher 가 한 오버레이로 통합.
- 사이드바(`TodayAgendaPanel`) 재작성 = 세로 날짜 리스트. 한 번에 1년 창, 헤더 `2026년 [<][오늘][>]` 로 연도 이동·오늘 복귀. 일정만 풀 행(클릭=열기), 할일·노트·일기는 헤더 우측 아이콘+숫자 배지. 날짜("일 요일")는 일정 시각과 같은 좌측 거터 정렬, sticky 월 헤더(진한 밴드).
- **디자인 결정**: 다른 날 훑기를 무한 스크롤/더보기 버튼 대신 "1년 창 + 연도 네비"로 — 스크롤 점프 없이 예측 가능하고 헤더 한 곳에서 범위 이동. 사이드바는 "일정 훑기"가 본분이라 일정만 풀 노출, 할일·노트·일기는 신호(배지)만 (자세한 건 메인 대시보드·각 탭이 전담).
- 곁다리: 스크린샷 캡쳐 창 탐색을 제목 매칭 → 최전면 앱 창으로 (`scripts/find-window.swift`, dev 창 제목이 "app" 으로 남는 문제 우회).

## 2026-06-16

### PR #78 — "오늘" 대시보드 신설 + 기본 진입 변경 (SIMPLIFY 착수 1)

- **한 줄 임팩트**: 앱을 켜면 오늘 할 일이 한눈에 모입니다
- 앱을 켜면 안 쓰는 캘린더 그리드로 떨어지고 "오늘 나에게 중요한 것"이 어디에도 안 모이던 통증. 기본 진입을 "오늘" 대시보드로 변경 (빈 hash → today, 탭 순서 오늘/메모장/할일/캘린더/포트폴리오).
- 메인 = 오늘 고정 4블록 (오늘 일정 / 오늘·밀린 할 일(밀린 건 빨강) / 오늘의 루틴 / 이어서 쓸 노트) — 시간축 객체와 1:1. 상단에 할 일 빠른 추가.
- 사이드바 = 다가오는 일정 아젠다(TodayAgendaPanel) — 캘린더 그리드의 "날짜 훑기" 역할을 대신. 내일부터 30일 항목 있는 날만 모아 표시, 날짜 헤더 클릭 = 캘린더 탭 상세. 좌측 컬럼이 항상 있어 탭 왕복 시 안 흔들림.
- 메인을 아무 날짜로 바꾸는 대신 오늘 고정 + 사이드바가 미래 훑기를 맡는 분리 — 대시보드의 "오늘 초점"을 흐리지 않으려는 결정. 기존 컴포넌트/훅 재사용으로 신규 표면 최소화.
...
- 메모 탭 내 찾기(PR #62)에 FindBar ▸ 토글로 바꿀 내용 입력줄 + 바꾸기 / 전체 바꾸기 버튼이 펼쳐지는 2단 구조 (브라우저·VSCode 멘탈모델).
- 치환은 편집 모드 세 탭(본문/음성 기록/요약) textarea 에서 동작, 보기 모드는 바꾸기 줄 비활성 + "편집 모드에서 바꿀 수 있습니다" 안내.
- 치환을 기존 setDoc 경로(탭별 source, immediate flush)로 docHistory 에 단일 entry 로 올려 Cmd+Z 한 번에 undo (전체 바꾸기도 단일 entry).
- `replaceAllInText` 순수 함수 — 겹치지 않는 매치만 치환(겹침 데이터 유실 방지) + offset 안 밀림. 단위 테스트 8개.

### PR #64 — 날짜/시간 표시 포맷 통일

- **한 줄 임팩트**: 앱 전체 날짜/시간 표시가 한 컨벤션으로
- 호출처마다 제각각이던 날짜 표시(`MM/DD`·`YY/MM/DD`·`yyyy.MM.dd(EEE)`·`5월 6일 화요일`·`M/d`·raw ISO)를 단일 포맷으로 통일. 올해면 연도 생략, 올해 아니면 `YYYY.MM.DD(ddd)`.
- `lib/dates.ts` 에 정규 함수 2개 도입: `formatDisplayDate` (날짜) / `formatDisplayDateTime` (날짜+시각, 휴지통 삭제 시각용). 중복 함수 `formatMemoDateShort`·`formatDateTimeKo` 제거 후 흡수.
- 호출처 6종 통일 — 메모장 사이드바 카드 · 캘린더 사이드바 메모 · 캘린더 선택 날짜 헤더 · 메모 본문 메타 row · 휴지통(메모/포트폴리오) · todos 마감일(목록/휴지통) · portfolio 카드. 새 표시 위치는 함수만 호출하면 됨.
- 캘린더 선택 날짜 헤더는 자연어(`5월 6일 화요일`) → 점 포맷으로 변경해 전체 일관성 확보.
...
- 현재 탭(본문/음성기록/요약)에서 검색 — 다른 탭·메타는 제외. Enter/↓ 다음, Shift+Enter/↑ 이전, Cmd+G/Cmd+Shift+G 동일. 대소문자 토글 + 매치 카운트.
- 편집 모드는 textarea 선택으로 강조(focus 는 find 입력창 유지 → Enter 연타 순회, 메모 오염 X), 보기 모드는 보이는 텍스트 기준 매칭 → Range client rect 오버레이 박스로 강조.
- 곁들임: 날짜·시간 input 상하 방향키로 구간별 값 증감(앞선 main 직커밋 포함).
