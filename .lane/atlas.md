...
- 오늘(기본 진입): `#today` → 사이드바 `TodayAgendaPanel`(미니 월 그리드: 일정 점·다일·선택일 ring + 다가오는 일정 ≤10 + 일정 추가). 날짜 선택 시 메인창(`TodayPage`)이 그 날 **day-view 로 전환**(모달 아님 — 다른 탭과 동일한 사이드바→메인. `selectedDay` state, 기본=오늘, "오늘로" 복귀, 탭 재진입 리셋). 메인 2열: 할일|일정 / 노트|일기. 할일=할일탭과 동일 순서(`compareTaskDate`@`lib/taskSort.ts`) 상위 10·완료 숨김. 일정=`DayEvents`(보기·추가·편집·삭제). 일기=인라인 편집(`JournalEditor`, 자동저장). 루틴은 오늘 탭에서 제외(#routines 전담). 일정 표기 통일=내용-날짜-시간. ※`DayDetailModal` 폐기됨(편집 UI→`DayEvents`)
