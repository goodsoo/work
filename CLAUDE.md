...
- **회의록 폼 상태** (V0.5.3, V0.7.1 cacheKey 갱신): `useStateHistory` 를 4개 stack 으로 분리 — `body`(content) / `transcript` / `summary`(discussion_items + decisions + action_items) / `meta`(title + date + time + attendees). 각자 `cacheKey="${uid}:${stack}"` 로 module-level Map 에 보존 (V0.7.1 이전엔 path 기반 → 옛 메모 cache 침범 버그). 메모 전환 또는 페이지 전환 후 같은 메모로 돌아오면 value + history + canUndo 가 그대로 복원됨. 각 stack 의 `onCommit` 은 자기 필드만 partial patch → `useUpdateMeeting` (optimistic). pending commit 은 cacheKey transition 시 outgoing onCommit (옛 메모의 mutation) 에 deferred flush. title input 은 별도 useState draft + commit 시점 검사 — 사용자 typing 중엔 server data 가 덮어쓰지 않게 useEffect 에 focus 검사.
...
- **메모장 에디터** (V0.5.2): 제목(3xl) → 메타데이터(인라인) → 3-탭 [본문 / 회의 내용 / 요약]. 탭 row sticky(`top: 2.5rem`). 우측 액션(편집/보기 토글, 마크다운 복사, 삭제) compact 아이콘. 하단에 액션 X.
  ...
  - 단축키 (Tauri only): Cmd/Ctrl+Z (활성 탭 stack 의 undo — meta 는 native input undo), Cmd+1/2/3/4 페이지 탭, Cmd+[/] 메모 sub-tab 좌/우 cycling (본문 ↔ 회의 내용 ↔ 요약), Cmd+E 본문 탭일 때 편집/보기 토글 (옵시디안 패턴). 본문 탭 라벨 옆에 현재 모드 ("편집"/"보기") chip 표시. 브라우저는 시스템 단축키 충돌로 sub-tab 단축키 없음.
