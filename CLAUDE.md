...
- **요약 AI 흐름** (V0.6 + V0.7.3): Supabase Edge Function / 자동 요약 폐기 → **"Claude 프롬프트 복사" 헬퍼** 로 전환. `src/lib/clipboardPrompt.ts` 의 `buildClaudePrompt()` 가 메모를 보고 외부 Claude 에 줄 프롬프트 생성 (출력 형식 = `### 논의 사항` / `### 결정 사항` (합의·확정된 것만) / `### 액션 아이템` (`[담당자] 할 일 — 기한`) H3 섹션 마크다운). 사용자가 복사 → Claude 응답 → `SummaryModal` 에 paste → `summary` string 통째 적용. 옛 array 3분리 (`discussion_items`/`decisions`/`action_items`) 와 `claude-haiku` Edge Function 호출은 src 에 더 이상 없음.
